...
contract MyV2FlashLoan is FlashLoanReceiverBase {
  ...
  address payable public owner = 0x8406A51A0E1B5F52Ff61226773e6328e5Da5d964;
  ...
  */
    ...
    returns (bool)
  {
    ...
    for (uint i = 0; i < assets.length; i++) {
      ...
      IERC20(assets[i]).approve(address(LENDING_POOL), amountOwing);
        ...
        // Transfer any remaining tokens back to contract owner
        uint256 remainingWAVAX = WAVAX_ERC20.balanceOf(address(this));
        uint256 remainingQI = QI_ERC20.balanceOf(address(this));
        if (remainingWAVAX > 0) {
            WAVAX_ERC20.transfer(owner, remainingWAVAX);
        }
        if (remainingQI > 0) {
            QI_ERC20.transfer(owner, remainingQI);
        }
  ...
  function myFlashLoanCall() external {
