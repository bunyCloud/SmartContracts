...
import './aionInterface.sol'; //Aion Interface
...
contract DCA{
    ...
    function TokenToToken(address tokenToSell, address tokenToBuy, uint256 interval, uint256 amountToSell) public payable{
        ...
        uint256 tokens_bought = exchange.tokenToTokenSwapInput(amountToSell.sub(fee), 1, 1, block.timestamp, tokenToBuy);
        ...
        (uint256 aionID, address aionClientAccount) = aion.ScheduleCall{value:callCost}(block.timestamp + interval, address(this), 0, gasAmount, maxGasPrice, data, true);
    ...
    function TokenToETH(address tokenToSell, uint256 amountEther) internal returns(uint256 amountSold){
        ...
        address exchangeAddress = uniswapInstance.getExchange(tokenToSell);
        UniswapExchange exchange = UniswapExchange(exchangeAddress);
        amountSold = exchange.tokenToEthSwapOutput(amountEther, uint256(-1), block.timestamp);
